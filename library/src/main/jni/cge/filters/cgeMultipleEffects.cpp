	...
	bool CGEMutipleEffectFilter::initWithEffectString(const char* effectString)
	{
		...
		while(*ptr != '\0')
		{
			...
			*pBuffer = '\0';
			...
			{
				CGEDataParsingEngine::squareParser(ptr, this);
			}
            else if(strcmp(buffer, "double") == 0)
			{
				CGEDataParsingEngine::doubleParser(ptr, this);
			}
            else if(strcmp(buffer, "quad") == 0)
			{
				CGEDataParsingEngine::quadParser(ptr, this);
			}
            else if(strcmp(buffer, "quadflip") == 0)
			{
				CGEDataParsingEngine::quadFlipParser(ptr, this);
			}
            else if(strcmp(buffer, "heart") == 0)
			{
				CGEDataParsingEngine::heartParser(ptr, this);
			}
            else if(strcmp(buffer, "hexa") == 0)
			{
				CGEDataParsingEngine::hexaParser(ptr, this);
			}
            else if(strcmp(buffer, "zoom") == 0)
			{
				CGEDataParsingEngine::zoomParser(ptr, this);
			}
			else if(strcmp(buffer, "triple") == 0)
            {
                CGEDataParsingEngine::tripleParser(ptr, this);
            }
