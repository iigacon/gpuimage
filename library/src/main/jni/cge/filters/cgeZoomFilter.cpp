...
namespace CGE
{
	CGEZoomFilter::~CGEZoomFilter()
    	{
    		_clearMipmaps();
    	}
    
    	bool CGEZoomFilter::init()
    	{
    	...
    	void CGEZoomFilter::setBlurLevel(int value)
    	{
    		m_intensity = value;
    	...
    	void CGEZoomFilter::setIntensity(float value)
    	{
    		if(value <= 0.5f)
    		{
    	...
    	void CGEZoomFilter::_genMipmaps(int width, int height)
    	{
    		_clearMipmaps();
    	...
    	void CGEZoomFilter::_clearMipmaps()
    	{
    		GLuint texIDs[MAX_LERP_BLUR_INTENSITY];
    		for(int i = 0; i != MAX_LERP_BLUR_INTENSITY; ++i)
    	...
    	void CGEZoomFilter::render2Texture(CGEImageHandlerInterface* handler, GLuint srcTexture, GLuint vertexBufferID)
    	{
    		if(m_intensity <= 0)
    		{
    		...
    		if(m_texCache[0].texID == 0 || m_cacheTargetSize.width != sz.width || m_cacheTargetSize.height != sz.height || m_isBaseChanged)
    		{
    			...
    			CGE_LOG_INFO("CGEZoomFilter::render2Texture - Base Changing!\n");
    		}
    	...
    	void CGEZoomFilter::setMipmapBase(float value)
    	{
    		m_mipmapBase = value;
    	...
    	int CGEZoomFilter::_calcLevel(int len, int level)
    	{
