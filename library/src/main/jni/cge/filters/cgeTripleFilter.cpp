...
const static char* const s_fshScale = CGE_SHADER_STRING_PRECISION_H
(
...
void main() {
  ...
  gl_FragColor = texture2D(inputTexture2, dstPos);
...
namespace CGE
{
	CGETripleFilter::~CGETripleFilter()
    	{
    		_clearMipmaps();
    	}
    
    	bool CGETripleFilter::init()
    	{
    	...
    	void CGETripleFilter::setBlurLevel(int value)
    	{
    		m_intensity = value;
    	...
    	void CGETripleFilter::setIntensity(float value)
    	{
    		if(value <= 0.5f)
    		{
    	...
    	void CGETripleFilter::_genMipmaps(int width, int height)
    	{
    		_clearMipmaps();
    	...
    	void CGETripleFilter::_clearMipmaps()
    	{
    		GLuint texIDs[MAX_LERP_BLUR_INTENSITY];
    		for(int i = 0; i != MAX_LERP_BLUR_INTENSITY; ++i)
    	...
    	void CGETripleFilter::render2Texture(CGEImageHandlerInterface* handler, GLuint srcTexture, GLuint vertexBufferID)
    	{
    		if(m_intensity <= 0)
    		{
            ...
            m_program.sendUniformi("inputTexture2", 1);
    		...
    		if(m_texCache[0].texID == 0 || m_cacheTargetSize.width != sz.width || m_cacheTargetSize.height != sz.height || m_isBaseChanged)
    		{
    			...
    			CGE_LOG_INFO("CGETripleFilter::render2Texture - Base Changing!\n");
    		}
    	...
    	void CGETripleFilter::setMipmapBase(float value)
    	{
    		m_mipmapBase = value;
    	...
    	int CGETripleFilter::_calcLevel(int len, int level)
    	{
