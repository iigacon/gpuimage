...
namespace CGE
{
    ...
    void CGEHeartFilter::setBlurLevel(int value)
    {
        m_intensity = value;
    ...
    void CGEHeartFilter::_genMipmaps(int width, int height)
    {
        _clearMipmaps();
    ...
    void CGEHeartFilter::_clearMipmaps()
    {
        GLuint texIDs[MAX_LERP_BLUR_INTENSITY];
        for(int i = 0; i != MAX_LERP_BLUR_INTENSITY; ++i)
    ...
    void CGEHeartFilter::render2Texture(CGEImageHandlerInterface* handler, GLuint srcTexture, GLuint vertexBufferID)
    {
        ...
        if(m_texCache[0].texID == 0 || m_cacheTargetSize.width != sz.width || m_cacheTargetSize.height != sz.height || m_isBaseChanged)
        {
            ...
            CGE_LOG_INFO("CGEHeartFilter::render2Texture - Base Changing!\n");
        }
    ...
    void CGEHeartFilter::setMipmapBase(float value)
    {
        m_mipmapBase = value;
    ...
    int CGEHeartFilter::_calcLevel(int len, int level)
    {
